...
export default function RecipeTechnical() {
  ...
  const sanitizeNumericData = useCallback((data) => {
    console.log('🧹 [SANITIZE] Iniciando sanitização com correção para assembly_config');
    ...
    recipeFields.forEach(field => {
      if (sanitized[field] !== undefined) {
        ...
        if (original !== sanitized[field]) {
          console.log(`🧹 [SANITIZE] Recipe.${field}: "${original}" → ${sanitized[field]}`);
        }
      }
    });
    ...
    if (sanitized.processes && Array.isArray(sanitized.processes)) {
      sanitized.processes.forEach((prep, prepIdx) => {
        console.log(`🧹 [SANITIZE] Sanitizando preparação ${prepIdx}: ${prep.title}`);
        ...
        prepFields.forEach(field => {
          if (prep[field] !== undefined) {
            ...
            if (original !== prep[field]) {
              console.log(`🧹 [SANITIZE] prep[${prepIdx}].${field}: "${original}" → ${prep[field]}`);
            }
          }
        });
        ...
        if (prep.assembly_config && typeof prep.assembly_config === 'object') {
          console.log(`🧹 [SANITIZE] Sanitizing assembly_config da prep ${prepIdx} (como STRINGS):`, prep.assembly_config);
          ...
          assemblyStringFields.forEach(field => {
            if (prep.assembly_config[field] !== undefined) {
              ...
              console.log(`🧹 [SANITIZE] assembly_config.${field}: ${original} → "${prep.assembly_config[field]}" (como STRING)`);
            }
          });
            ...
            ingFields.forEach(field => {
              if (ing[field] !== undefined) {
                ...
                if (original !== ing[field] && original !== '' && original !== null) { // Only log if change or not empty
                  console.log(`🧹 [SANITIZE] ing[${ingIdx}].${field}: "${original}" → ${ing[field]}`);
                }
              }
            });
          });
            ...
            scFields.forEach(field => {
              if (sc[field] !== undefined) {
                ...
                if (original !== sc[field] && original !== '' && original !== null) { // Only log if change or not empty
                  console.log(`🧹 [SANITIZE] sc[${scIdx}].${field}: "${original}" → ${sc[field]}`);
                }
              }
            });
          });
    ...
    console.log('✅ [SANITIZE] Sanitização com correção para assembly_config finalizada');
    return sanitized;
  }, [parseNumericValue]);
  ...
  const handleRecalculate = useCallback((currentRecipeData, currentPreparationsData) => {
    console.log('🔄 [CALC] Starting recalculation with provided preparations:', currentPreparationsData);
    ...
    try {
      ...
      console.log('💾 [CALC] Preserving current cuba_weight (string input):', currentCubaWeightString);
      ...
      if (currentCubaWeightString !== undefined && currentCubaWeightString !== null) {
        ...
        console.log('🔒 [CALC] Cuba weight restored to (string):', updatedRecipe.cuba_weight);
      } else {
      ...
      console.log('🔄 [CALC] Values calculated, updating states for real-time feedback.');
  ...
  const handleSubComponentWeightChange = useCallback((prepId, subComponentId, newWeight) => {
    console.log(`📦 [SUB-COMPONENT] Peso alterado: ${newWeight} para componente ${subComponentId}`);
    ...
    setPreparationsData(prev => prev.map(prep => {
      if (prep.id === prepId) {
        ...
        console.log(`🏗️ [AUTO-CALC] Peso total da montagem calculado: ${totalWeight}kg`);
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      try {
        ...
        await loadCategoryTypes(); // This loads category types and allCategories

        if (!id || id === 'new') {
          console.log("🔄 [LOAD] Criando nova receita...");
          initializeNewRecipe(); // Call the dedicated function for new recipe setup
          setSearchQuery(''); // Clear search query for new recipe
        } else {
          console.log("🔄 [LOAD] Carregando receita:", id);
  ...
  const loadUserConfiguration = async () => {
    try {
      ...
      // if (user && user.recipe_config) {
      //   if (user.recipe_config.selected_category) {
      //     setSelectedCategory(user.recipe_config.selected_category);

      //     const parentCategory = allCategories.find(cat => cat.id === user.recipe_config.selected_category);
      //     if (parentCategory) {
      //       const subs = allCategories.filter(cat =>
      //         cat.parent_id === user.recipe_config.selected_category && cat.level === 2
      //       );
  ...
  const loadRecipe = async (id) => {
    try {
      setLoading(true);
      console.log('[RecipeTechnical] 🔍 Carregando receita:', id);
      ...
      console.log('[RecipeTechnical] 📦 Receita carregada RAW do banco:', recipe);
      ...
      console.log('[RecipeTechnical] 🔍 Campo processes do banco:', recipe.processes);
      console.log('[RecipeTechnical] 🔍 Campo ingredients antigo (IGNORADO):', recipe.ingredients); // Log for verification
      ...
      if (recipe.processes && Array.isArray(recipe.processes) && recipe.processes.length > 0) {
        ...
        console.log('[RecipeTechnical] ✅ Carregando do campo PROCESSES (novo formato)');
      } else {
        // Se não tem processes, começar com array vazio
        processedPreparations = [];
        console.log('[RecipeTechnical] 📝 Campo processes vazio - iniciando com array vazio');
      }
      ...
      console.log('[RecipeTechnical] 🔧 Dados finais processados:');
      console.log('- Receita:', processedRecipe);
      console.log('- Preparações (apenas de processes):', processedPreparations);
      console.log('[RecipeTechnical] ✅ Receita carregada com sucesso!');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.log('📝 Dados da receita ANTES do cálculo:', recipeData);
      console.log('📝 Preparações ANTES do cálculo:', preparationsData);
      ...
      console.log('🧹 Preparações após limpeza:', cleanedPreparations);

      // Calcular métricas com DEBUG
      console.log('\n🧮 ========== INICIANDO CÁLCULOS COM LOGS DETALHADOS ==========');
      ...
      console.log('\n💾 ========== DADOS CALCULADOS PARA SALVAMENTO ==========');
      console.log('📊 Receita calculada:', updatedRecipe);
      console.log('📊 Preparações calculadas:', updatedPreparations);
      ...
      console.log('💾 Dados finais sanitizados que serão salvos no banco:', dataToSave);

      let savedRecipe;
      if (currentRecipeId && currentRecipeId !== 'new' && currentRecipeId !== 'null') { // Check if it's an existing recipe ID
        console.log(`✏️ Atualizando receita existente com ID: ${currentRecipeId}`);
      ...
      console.log('📊 Atualizando recipeData para:', updatedRecipe);
      console.log('📊 Atualizando preparationsData para:', updatedPreparations);
      ...
      toast({
        ...
        description: err.response?.data?.message || err.message || "Erro interno"
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDeleteProcess = (prepId) => {
    console.log('[RecipeTechnical] 🗑️ Deletando processo:', prepId);

    setPreparationsData(prevPreps => {
      const updatedPreparations = prevPreps.filter(prep => prep.id !== prepId);
      console.log('[RecipeTechnical] 📝 Preparações após exclusão:', updatedPreparations);
  ...
  const handleCubaWeightChange = (e) => {
    const value = e.target.value;
    console.log('🏺 [CUBA] Peso da cuba alterado para:', value);
  ...
  const copyRecipeData = () => {
    ...
    console.log("🍽️ [COPY] === INICIANDO CÓPIA DE RECEITA ===");
    console.log("🍽️ [COPY] Receita fonte:", selectedSourceRecipe.name);
    console.log("🍽️ [COPY] Nível selecionado:", selectedStageLevel);
    ...
    try {
      if (selectedStageLevel === 'complete') {
        ...
        if (selectedSourceRecipe.ingredients && Array.isArray(selectedSourceRecipe.ingredients)) {
          copiedIngredients = selectedSourceRecipe.ingredients.map(ing => {
            console.log("🍽️ [COPY] Ingrediente original:", ing);
            ...
            console.log("🍽️ [COPY] Pesos extraídos:", {
              weightFrozen, weightThawed, weightRaw, weightClean,
              weightPreCooking, weightCooked, weightPortioned
            });
            ...
            console.log("🍽️ [COPY] Ingrediente copiado:", copiedIngredient);
      ...
      console.log("🍽️ [COPY] Ingredientes finais copiados:", copiedIngredients);
      console.log("🍽️ [COPY] Sub-componentes finais copiados:", copiedSubComponents);
      console.log("🍽️ [COPY] Tipos de processo final:", newPreparationProcesses);
      ...
      console.log("🍽️ [COPY] Nova preparação criada:", newPreparation);
  ...
  const renderProcessTable = (prep, prepIndex) => {
    console.log(`🎨 [RENDER] renderProcessTable called for prep ${prepIndex}:`, prep);
    ...
    console.log(`🎨 [RENDER] Prep ${prepIndex} has ${ingredients.length} ingredients`);
          ...
          <tbody>
            {!hasIngredients && !isRecipeProcess && !prep.sub_components?.length > 0 ? ( 
              ...
              ingredients.map((item, itemIndex) => {
                console.log(`🎨 [RENDER] Rendering ingredient ${itemIndex}:`, item);
  ...
  const renderPreparationCards = () => {
    console.log("🎨 [RENDER] renderPreparationCards called with preparations:", preparationsData);
    ...
    return preparationsData.map((prep, prepIndex) => {
      console.log(`🎨 [RENDER] Rendering preparation ${prepIndex}:`, prep);
  ...
  const updatePreparationItem = (prepIndex, itemIndex, field, value) => {
    console.log(`🔄 [UPDATE] updatePreparationItem called: prep=${prepIndex}, item=${itemIndex}, field=${field}, value=${value}`);
    ...
    setPreparationsData(prev => {
      console.log(`🔄 [UPDATE] Current preparations:`, prev);
      ...
      console.log(`🔄 [UPDATE] Updated item:`, itemToUpdate);
  ...
  const addItemToPreparation = (itemDataFromModal, isIngredient = true, prepIndex = 0) => {
    setPreparationsData(prev => {
      ...
      if (isIngredient) {
        ...
        if (sourcePrep) {
          console.log("📊 [ADD ITEM] Preparação fonte encontrada:", sourcePrep.title);
        ...
        console.log("📊 [ADD ITEM] Valores finais - Rendimento:", inputYieldWeight, "Custo:", inputTotalCost);
        ...
        console.log("📊 [ADD ITEM] Sub-componente criado:", newSubComponent);
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    if (parentCategory) {
                      const subs = allCategories.filter(cat =>
                        cat.parent_id === value && cat.level === 2
                      );
                      setFilteredSubcategories(subs);
                    }
                    setIsDirty(true);
